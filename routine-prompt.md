...
resumo em HTML com a logo da Análise Macro e enviá-lo por e-mail.

## Passos

1. **Localize o `.txt` mais recente.** Liste `data/focus_*.txt` e pegue
   o de data mais alta. Se não houver nenhum, pare sem enviar e-mail — o
   Action não rodou.
...
7. **Envie o e-mail** pelo conector do Gmail (a ferramenta de envio de
   e-mail do Gmail autorizada no ambiente da Routine). Se o conector do
   Gmail não estiver disponível, pare sem enviar e registre o motivo no
   transcript — não tente outro meio de envio.
   - Assunto: `Resumo Focus — AAAA-MM-DD`
   - Corpo: o HTML montado no passo 5.
   - Destinatário: `analisemacro.cloud@gmail.com` (para mais de um,
     separe por vírgula: `fulano@exemplo.com, ciclano@exemplo.com`).

## Falhas

Em qualquer cenário abaixo, pare sem enviar e-mail. O motivo aparece no
